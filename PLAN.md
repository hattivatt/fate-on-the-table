...
- привязка зоны к одному или нескольким ситуативным аспектам — **реализовано структурно**: ситуативный аспект хранит `zoneIds: string[]` (массив id зон с живого борда, `scene.flags["fate-core-official"].situation_aspects` + `readConflictBoard(scene).zones`), текстовый суффикс `"Аспект (Зона)"` для зон больше не используется и мигрируется в `SituationAspectSync.migrateZoneSuffixes`; менеджер предлагает мульти-выбор чекбоксов, виджет маркирует зональные аспекты префиксом `◈` (`SA_ZONE_MARKER` в `scripts/situationAspectZones.js` / `saAspectLine`);
...
- связь board с `game.combat` lifecycle и переключением между несколькими
  конфликтами.
- **пиктограммы-маркеры типов аспектов в SA-виджете по образцу зональной привязки `◈`** (`scripts/situationAspectZones.js` `SA_ZONE_MARKER` / `saAspectLine`): реализован пока ТОЛЬКО зональный маркер `◈` (`SA_ZONE_MARKER` / `saAspectLine`); пиктограммы по типам аспектов (последствия, персонажные и т.п.) — будущая работа, механизм на примере маркера — по константе-маркеру на тип, дешёвое расширение отрисовки строк виджета без изменения данных сцены.
- **рефакторинг-бэклог: тестируемость UI-менеджеров** — ленивый guard `ApplicationV2` (как в `ConflictManager`) для `FatePointManager`/`SituationAspectManager`/`ConflictZoneEditor` + вынос pure-логики (`renderContent`, действия) в тестируемые модули — сейчас у трёх менеджеров 0 тестов (~45% кода непокрыто);
- **рефакторинг-бэклог: декомпозиция гигантов** — `ConflictBoardSync.js` (1669 строк, `syncConflictBoardNow` 230 строк — отделить pure-подготовку от I/O), `ConflictManager.js` (1579 строк, менеджер+3 диалога), `ConflictInteractions.js` (1514 строк, 4 меню+drag-drop+хит-тесты);
- **рефакторинг-бэклог: стилевой рефакторинг (низкий приоритет)** — `Handlebars`-шаблоны вместо HTML-конкатенации, `DOMPurify` вместо ручного `escapeHtml`, отказ от прототип-патчей где возможно — референс: паттерны `fate-core-official`.
